...
export function formatTimeStampString(when = new Date(), type = TYPE.FORMAT_TYPE_NORMAL, is24 = true) {
    ...
    if (then && then.toString() === 'Invalid Date') {
        throw 'Function: formatTimeStampString: Date arguments is invalid.'
    }
    
    let now = null
    let nowmillis
    
    let sameType = (type == FormatTypeLast);
    FormatTypeLast = type;
    ...
    let weekStart = now.getYearDay() - now.getDay()
    let isBeforeYear = then.getFullYear() < now.getFullYear()
    let isThisYear = then.getFullYear() === now.getFullYear() && then.getYearDay() <= now.getYearDay()
    let isToday = isThisYear && (then.getYearDay() === now.getYearDay())
    let isYesterDay = isThisYear && (then.getYearDay() === now.getYearDay() - 1)
    let isThisWeek = (isThisYear && (then.getYearDay() >= weekStart && then.getYearDay() < now.getYearDay()))
    ...
    switch (type) {
        ...
        case TYPE.FORMAT_TYPE_PERSONAL_FOOTPRINT: 
            if (isToday) {
                let offsetMilliSecounds = nowmillis - when
                let returnValue
        ...
        case TYPE.FORMAT_TYPE_CALENDAR_APPWIDGET:
            if (now.getFullYear() === then.getFullYear()) {
                ...
                FormatResultLast = then.format(mc_pattern_year_month)
                return FormatResultLast
            }
        ...
        case TYPE.FORMAT_TYPE_CALL_LOGS_NEW: 
            ...
            currentTime = is24 ? then.format(mc_pattern_hour_minute) : then.format(mc_pattern_hour_minute_12)
            if (isToday) {
